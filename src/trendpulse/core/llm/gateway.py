...
class LLMGateway:
    ...
    async def chat(
        ...
        *,
        ...
        llm_request_timeout_seconds = float(getattr(settings, "llm_request_timeout_seconds", 30.0))
        llm_retry_backoff_seconds = float(getattr(settings, "llm_retry_backoff_seconds", 0.25))
        llm_circuit_breaker_fail_threshold = int(getattr(settings, "llm_circuit_breaker_fail_threshold", 3))
        llm_circuit_breaker_open_seconds = float(getattr(settings, "llm_circuit_breaker_open_seconds", 20.0))
        ...
        for provider in ordered:
            ...
            if open_until > now:
                logger.warning(f"Skipping provider={provider} circuit=open until={open_until:.3f}")
                continue

            max_attempts = llm_max_retries + 1
            for attempt in range(1, max_attempts + 1):
                try:
                    logger.info(f"Trying provider={provider} model={decision.model} attempt={attempt}/{max_attempts}")
                    ...
                    last_error = ProviderTimeoutError(f"Provider timeout provider={provider} attempt={attempt}: {e}")
                except Exception as e:
                    last_error = e

                is_timeout = isinstance(last_error, ProviderTimeoutError)
                ...
                logger.warning(
                    f"Provider failed provider={provider} attempt={attempt}/{max_attempts} "
                    f"fails={current_fails} err={err_type}"
                )

                if current_fails >= llm_circuit_breaker_fail_threshold or is_timeout:
                    self._circuit_open_until[provider] = monotonic() + llm_circuit_breaker_open_seconds
                    logger.warning(f"Circuit opened provider={provider} for={llm_circuit_breaker_open_seconds}s")
                    break

                if attempt < max_attempts:
                    backoff = llm_retry_backoff_seconds * (2 ** (attempt - 1))
